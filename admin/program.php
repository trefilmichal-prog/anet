    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            if ($action === 'delete') {
                ...
                $deleteStmt = $db->prepare('DELETE FROM program_items WHERE id = :id');
                $deleteStmt->execute(array(':id' => $id));
                $message = 'Položka programu byla odstraněna.';
                $editRow = null;
            } else {
            if ($title === '') {
                        ...
                        <tr>
                            ...
                            <td data-label="Akce">
                                <div class="admin-actions">
                                    <a class="admin-button admin-button--secondary" href="program.php?edit=<?php echo (int) $row['id']; ?>">Upravit</a>
                                    <form method="post" onsubmit="return confirm('Opravdu chcete tuto položku programu odstranit?');">
